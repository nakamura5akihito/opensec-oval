...
import io.opensec.oval.core.repository.morphia.OvalDatastore;
import io.opensec.oval.model.definitions.DefinitionType;
import io.opensec.oval.model.definitions.DefinitionsElementAssoc;
...
//    }
    ...
    {
        if (_DATABASE_ == null) {
            _DATABASE_ = OpensecOvalContext.repository().getBean( OvalDatastore.class );
        }
    ...
    private static OvalXmlMapper _getXmlMapper()
    {
        if (_XML_MAPPER_ == null) {
            _XML_MAPPER_ = OpensecOvalContext.repository().getXmlMapper();
        }
